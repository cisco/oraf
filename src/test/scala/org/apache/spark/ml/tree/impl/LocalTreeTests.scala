...
private[impl] object LocalTreeTests extends Logging {
  ...
  private[impl] def finalizeTree(
      rootNode: OptimizedNode,
      algo: OldAlgo.Algo,
      numClasses: Int,
      numFeatures: Int,
      parentUID: Option[String]): OptimizedDecisionTreeModel = {
    parentUID match {
      case Some(uid) =>
        if (algo == OldAlgo.Classification) {
          new OptimizedDecisionTreeClassificationModel(uid, rootNode, numFeatures = numFeatures,
            numClasses = numClasses)
        } else {
          new OptimizedDecisionTreeRegressionModel(uid, rootNode, numFeatures = numFeatures)
        }
      case None =>
        if (algo == OldAlgo.Classification) {
          new OptimizedDecisionTreeClassificationModel(rootNode, numFeatures = numFeatures,
            numClasses = numClasses)
        } else {
          new OptimizedDecisionTreeRegressionModel(rootNode, numFeatures = numFeatures)
        }
    }
  }
  ...
  private[impl] def train(
    ...
    val initialRoot = OptimizedLearningNode.emptyNode(nodeIndex = 1)
    // Fit tree
    val rootNode = (new LocalDecisionTree).fitNode(treeInput, instanceWeights,
