...
class LocalTreeUtilsSuite extends SparkFunSuite {
  ...
  test("transposeSelectedFeatures") {
    {
      val data = Array(Array(1, 2, 3), Array(4, 5, 6), Array(7, 8, 9))
      val selected = Array(1)

      val transposed = LocalDecisionTreeUtils.transposeSelectedFeatures(data, selected)
      val expected = Array(Array(2, 5, 8))
      transposed.zip(expected).foreach { case (resultCol, expectedCol) =>
        assert(resultCol.sameElements(expectedCol), s"Result column" +
          s"${resultCol.mkString(", ")} differed from expected col ${expectedCol.mkString(", ")}")
      }
    ...
    {
      val data = Array(Array(1, 2, 3), Array(4, 5, 6), Array(7, 8, 9))
      val selected = Array(0, 2)

      val transposed = LocalDecisionTreeUtils.transposeSelectedFeatures(data, selected)
      val expected = Array(Array(1, 4, 7), Array(3, 6, 9))
      transposed.zip(expected).foreach { case (resultCol, expectedCol) =>
        assert(resultCol.sameElements(expectedCol), s"Result column" +
          s"${resultCol.mkString(", ")} differed from expected col ${expectedCol.mkString(", ")}")
      }
